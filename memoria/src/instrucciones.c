...
void cargar_y_enviar_instrucciones(char* vector_instrucciones, int posicion_caracter_fin){
    ...
    for(int i=0; i < posicion_caracter_fin; i++){
        ...
        if(vector_instrucciones[i] == ' '){
            ...
            do{
                (asignar_a_operando(operacion, vector_instrucciones[i], contador_operando));

                i++;

                contador_operando ++;

            } while(vector_instrucciones[i+1] != '\n');
        ...
        if(vector_instrucciones[i] == '\n'){
            ...
            operacion->operando_de_operacion[i+1] = '\0';

            eliminar_paquete(paquete);

            paquete = crear_paquete();

            crear_stream(stream, operacion->codigo_de_operacion, operacion->operando_de_operacion);
            ...
            vaciar_string(operacion->codigo_de_operacion, i);

            vaciar_string(operacion->operando_de_operacion, i);
        ...
        asignar_a_codop(operacion, vector_instrucciones[i], contador_codop);
    ...
    //enviar_mensaje("envio_finalizado", socket);

    printf("Envio de instrucciones finalizado");
...
void asignar_a_codop(OPERACION* operacion, char caracter_codop, int contador){
    ...
    operacion->codigo_de_operacion[contador] = caracter_codop;

}

void asignar_a_operando(OPERACION* operacion, char caracter_operando, int contador){

    operacion->operando_de_operacion[contador] = caracter_operando;
